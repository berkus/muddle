...
class Directory(object):
    ...
    def __init__(self, where, stay_on_error=False, show_pushd=True,
        ...
        if show_pushd:
            logger.warning('++ pushd to %s'%self.where)
    ...
    def close(self):
        ...
        if self.show_popd:
            logger.warning('++ popd to  %s'%self.start)
    ...
    def __exit__(self, etype, value, tb):
        if tb is None:
            ...
            if self.show_popd:
                logger.warning('** Oops, an exception occurred - %s tidying up'%self.__class__.__name__)
    ...
    def close(self, delete_tree):
        ...
        if delete_tree:
            if self.show_dirops:
                ...
                logger.warning('++ rmtree   %s'%self.where)
