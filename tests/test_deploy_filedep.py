...
# A simple build description using filedep deployment
...
def describe_to(builder):
    ...
    filedep.deploy(builder, 'targetdir', deployment, [role1, role2])
...
def describe_to(builder):
    ...
    filedep.deploy(builder, 'targetdir', deployment, [role2, role1])
...
<instructions>
  ...
  <!-- We're not going to do that for our filedep test, as it would
       need sudo
  <chown>
    <filespec>
      <root>/</root>
  ...
  <!-- ...but filedep deployments need sudo for mknod
  <mknod>
...
def make_old_build_tree():
    """Make a build tree that does filedep deployment, and use/test it
    """
    with NewDirectory('build.old') as d:
        ...
        shell('ls -lR deploy')
        dt = DirTree('deploy')
        dt.assert_same_as_list(['  everything/',
                                '    bin/',
                                '      program1*',
                                '      program2*',
                                '    dev/',
        ...
        # And the MUDDLE_TARGET_LOCATION environment variable should be set
        # for the packages in the appropriate roles
        text = captured_muddle(['query', 'env', 'package:*{role1}'])
        if text.find('MUDDLE_TARGET_LOCATION=targetdir') == -1:
            raise GiveUp('Cannot see correct MUDDLE_TARGET_LOCATION in role1 environment:\n%s'%text)
        text = captured_muddle(['query', 'env', 'package:*{role2}'])
        if text.find('MUDDLE_TARGET_LOCATION=targetdir') == -1:
            raise GiveUp('Cannot see correct MUDDLE_TARGET_LOCATION in role2 environment:\n%s'%text)
