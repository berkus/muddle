#! /usr/bin/env python
"""Test collecting deployment support

    $ ./test_deploy_collect.py [-keep]
...
<instructions>
  ...
  <!-- We're not going to do that for our collecting test, as it would
       need sudo
  <chown>
    <filespec>
      <root>/</root>
  ...
  <!-- Drat. Collecting deployment tries to sudo for mknod, just in case
       of corner conditions. Drat.
  <chmod>
    <filespec>
  ...
  <!-- ...but collecting deployments don't support mknod
  <mknod>
...
def make_old_build_tree():
    """Make a build tree that does collecting deployment, and use/test it
    """
